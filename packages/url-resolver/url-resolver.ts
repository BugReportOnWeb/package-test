...
interface SentinelConfig {
  visual: {
    sitemap: { enabled: boolean; path: string };
    csv: { enabled: boolean; path: string; column: string };
    pages: Array<{ path: string; name: string }>;
  };
  migration: {
    sitemaps: { enabled: boolean; sourcePath: string; targetPath: string };
    csv: { enabled: boolean; path: string; sourceColumn: string; targetColumn: string };
    pairs: Array<{ sourceUrl: string; targetUrl: string; name: string }>;
  };
}
 ...
 */
function removeCommentKeys(obj: any): any {
 ...
 */
function loadConfig(configPath?: string): SentinelConfig {
  const resolvedPath = configPath ?? join(process.cwd(), 'sentinel.config.json');
  if (!existsSync(resolvedPath)) {
    throw new Error(
      `[sentinel:resolver] sentinel.config.json not found at ${resolvedPath}.`
    );
  }
  try {
    const raw = readFileSync(resolvedPath, 'utf-8');
    const parsed = JSON.parse(raw);
    return removeCommentKeys(parsed) as SentinelConfig;
  } catch (err) {
 ...
 */
function urlToPage(fullUrl: string): VisualPage {
 ...
 */
async function readSitemapContent(pathOrUrl: string): Promise<string> {
 ...
 */
async function resolveSitemap(pathOrUrl: string, visited = new Set<string>()): Promise<string[]> {
 ...
 */
async function parseSitemap(pathOrUrl: string): Promise<string[]> {
 ...
 */
function pairSitemapUrls(sourceUrls: string[], targetUrls: string[]): MigrationPair[] {
  ...
  if (unmatched.length > 0) {
    console.warn(
      `[sentinel:resolver] ${unmatched.length} path(s) in source sitemap have no match in target sitemap:\n` +
 ...
 */
export async function resolveVisualUrls(configPath?: string): Promise<VisualPage[]> {
  ...
  if (csv.enabled) {
    ...
    if (colIndex === -1) {
      throw new Error(
        `[sentinel:resolver] Column "${csv.column}" not found in CSV headers: [${headers.join(', ')}]`
      );
    }
 ...
 */
export async function resolveMigrationUrls(configPath?: string): Promise<MigrationPair[]> {
  const config = loadConfig(configPath);
  ...
  if (csv.enabled) {
    ...
    if (sourceColIndex === -1) {
      throw new Error(
        `[sentinel:resolver] Source column "${csv.sourceColumn}" not found in CSV headers: [${headers.join(', ')}]`
      );
    }
    if (targetColIndex === -1) {
      throw new Error(
        `[sentinel:resolver] Target column "${csv.targetColumn}" not found in CSV headers: [${headers.join(', ')}]`
      );
    }
