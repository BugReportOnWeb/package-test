...
async function askGeminiForSelector(
  ...
  try {
    const cleaned = responseText
      ...
      .replace(/\s*```$/i, '')
      .trim();
    return JSON.parse(cleaned) as GeminiHealResponse;
  } catch {
...
function resolveLocator(page: Page, locatorCode: string): Locator | null {
  try {
...
export async function healLocator(
  ...
  if (!apiKey) {
    throw new Error(
      '[sentinel:healer] GEMINI_API_KEY is not set. ' +
  ...
  const geminiResponse = await askGeminiForSelector(
    page,
    description,
    originalSelector,
    resolvedPromptPath,
  ...
  if (resolvedLocator) {
    ...
    if (count === 0 && geminiResponse.fallbackLocatorCode) {
      console.warn(
        `[sentinel:healer] Primary matched 0 elements, trying fallback: ${geminiResponse.fallbackLocatorCode}`
      );
 ...
 */
export async function withHealing(
  page: Page,
  locator: Locator,
  ...
  try {
    ...
    console.warn(
      `[sentinel:healer] Original locator failed: "${options.originalSelector}"`,
    ...
    if (!healResult.healed || !healResult.locator) {
      throw new Error(
        `[sentinel:healer] Could not heal broken selector "${options.originalSelector}". ` +
        `Gemini reasoning: ${healResult.reasoning}`
      );
    }

    console.log(
      `[sentinel:healer] Retrying action with healed locator: ${healResult.newSelector}`
    );
