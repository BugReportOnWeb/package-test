 ...
 */
function loadMaskSelectors(additionalMasks = [], pageUrl, masksPath) {
    try {
            ...
            for (const rule of config.maskingRules) {
                try {
                    ...
                    if (regex.test(pageUrl)) {
                        console.log(`[sentinel:stabilizer] Applying page-specific masks for pattern "${rule.pattern}" (${rule.description})`);
                        pageSpecificSelectors.push(...rule.selectors);
                    }
                }
                catch {
                    console.warn(`[sentinel:stabilizer] Invalid regex pattern in maskingRules: "${rule.pattern}". Skipping.`);
                }
            }
        }
        return [
        ...
        console.warn('[sentinel:stabilizer] Could not load sentinel.masks.json. Using additionalMasks only.');
        return additionalMasks;
    }
}
/** Injects CSS that kills all animations/transitions and masks dynamic elements with pink blocks. */
async function injectStabilizingCSS(page, maskSelectors) {
    const maskRules = maskSelectors.length > 0
        ? `
${maskSelectors.join(',\n')} {
...
async function mockBrowserClock(page) {
...
async function triggerLazyLoadScroll(page, scrollStep, scrollDelay) {
 ...
 */
export async function stabilizePage(page, options = {}) {
    const { additionalMasks = [], pageUrl, masksPath, mockClock = true, triggerLazyLoad = true, scrollStep = 300, scrollDelay = 100, } = options;
    if (mockClock) {
